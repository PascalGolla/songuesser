...
const FUZZY_LENGTH_FACTOR = 0.7;
const FUZZY_SUGGESTION_FACTOR = 0.7;
...
function calculateFuzzyMatchPartsScore(fuzzyMatchParts: FuzzyMatchPart[]) {
    ...
    const fullLength = fuzzyMatchParts
        .map((part) => part.content)
        .join("").length;
    ...
    return (
        (inputLength +
            fuzzyLength * FUZZY_LENGTH_FACTOR +
            suggestionLength * FUZZY_SUGGESTION_FACTOR) /
        fullLength
    );
}
